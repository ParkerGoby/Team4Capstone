...
import { divisionQuiz5Complete } from "../divisionQuiz5Complete.js";
import "../Quizzes.css";
...
class Quiz extends React.Component {
  ...
  constructor(props) {
    ...
    var random = Math.floor(Math.random() * 482);
    this.state = {
      nr: random,
      showButton: false,
  ...
  pushData(nr) {
    ...
    if (correctlyAnswered.length > 0){
      while (correctlyAnswered.includes(random) || nr === random){
        random = Math.floor(Math.random() * 482);
      }
    }
                ...
                <Modal.Footer>
                  ...
                  >
                    <div className="d-grid gap-2">
                      <Button
                        variant="primary"
                         onClick={() => {this.setState({ showModal: false }); divisionQuiz5Complete ();}}
                      >
                        Collect Badge
                      </Button>
                    </div>
