...
import "../Quizzes.css";
import {additionQuiz6Complete} from "../quizCompleteadd6";
...
class Quiz extends React.Component {
  ...
  constructor(props) {
    ...
    var random = Math.floor(Math.random() * 10201);
    this.state = {
      nr: random,
      showButton: false,
  ...
  pushData(nr) {
    ...
    if (correctlyAnswered.length > 0){
      while (correctlyAnswered.includes(random) || nr === random){
        random = Math.floor(Math.random() * 10201);
      }
    }
                ...
                <Modal.Footer>
                  ...
                  >
                    <div className="d-grid gap-2">
                      <Button
                        variant="primary"
                        onClick={() => {this.setState({ showModal: false });additionQuiz6Complete();}}
                      >
                        Collect Badge
                      </Button>
                    </div>
